...
export default function ParadasPage() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen">
        <p className="text-lg text-gray-500">Cargando paradas...</p>
      </div>
    )
  }
  ...
  return (
    <div className="h-screen flex flex-col md:flex-row overflow-hidden bg-gray-50">
      ...
      <aside className="w-full md:w-96 h-auto md:h-full overflow-y-auto bg-white border-r shadow-sm p-4">
        <div className="mb-4">
          ...
          <p className="text-xs text-gray-500 mt-2">
            Total de puntos: <strong>{paradas.length}</strong>
          </p>
        </div>
      ...
      {/* Mapa Principal */}
      <main className="flex-1 h-[400px] md:h-full">
        <Card className="h-full rounded-none md:rounded-lg m-0 md:m-4 overflow-hidden">
          <MapaLeaflet
            paradas={paradas}
