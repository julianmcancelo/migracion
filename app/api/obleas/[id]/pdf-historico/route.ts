...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    console.log('📄 Generando PDF solo con datos de la OBLEA (no inspección)')
    ...
    doc.setFontSize(11)
    doc.setFont('helvetica', 'normal')
    doc.text(`Transporte ${String(habilitacion.tipo_transporte || 'N/A')}`, 105, yPos, {
      align: 'center',
    })
    ...
    // ==================== FOTO DE LA OBLEA COLOCADA ====================
    if (oblea.path_foto && oblea.path_foto !== '') {
      // Nueva página para la foto
      doc.addPage()
      yPos = 20
      ...
      doc.setFontSize(11)
      doc.setFont('helvetica', 'bold')
      doc.text('EVIDENCIA FOTOGRÁFICA - OBLEA COLOCADA', 105, yPos, { align: 'center' })

      yPos += 12

      // Foto centrada de la oblea
      try {
        doc.addImage(oblea.path_foto, 'JPEG', 40, yPos, 130, 98)
        yPos += 100
      } catch (e) {
        console.error('Error agregando foto de oblea:', e)
        doc.setFillColor(245, 245, 245)
        doc.rect(40, yPos, 130, 98, 'F')
        doc.setDrawColor(200, 200, 200)
        doc.rect(40, yPos, 130, 98)
        doc.setFontSize(10)
        doc.setTextColor(150, 150, 150)
    ...
    for (let i = 1; i <= totalPaginas; i++) {
      ...
      doc.setFont('helvetica', 'normal')
      doc.text(
        'Documento oficial con validez legal. Conservar como evidencia de habilitación.',
        40,
        287
      )
